...
var CryptoOps = __webpack_require__(67898);
...
class NestedAppAuthAdapter {
    ...
    fromNaaTokenResponse(request, response, reqTimestamp) {
        ...
        const idTokenClaims = AuthToken/* extractTokenClaims */.My(response.id_token, this.crypto.base64Decode);
...
class NestedAppAuthController {
    ...
    async acquireTokenInteractive(request) {
        this.eventHandler.emitEvent(EventType/* EventType */.B.ACQUIRE_TOKEN_START, BrowserConstants/* InteractionType */.X8.Popup, request);
        const atPopupMeasurement = this.performanceClient.startMeasurement(PerformanceEvent/* PerformanceEvents */.MX.AcquireTokenPopup, request.correlationId);
        atPopupMeasurement?.add({ nestedAppAuthRequest: true });
        try {
            const naaRequest = this.nestedAppAuthAdapter.toNaaTokenRequest(request);
            const reqTimestamp = TimeUtils/* TimeUtils */.R.nowSeconds();
    ...
    async acquireTokenSilentInternal(request) {
        this.eventHandler.emitEvent(EventType/* EventType */.B.ACQUIRE_TOKEN_START, BrowserConstants/* InteractionType */.X8.Silent, request);
        const ssoSilentMeasurement = this.performanceClient.startMeasurement(PerformanceEvent/* PerformanceEvents */.MX.SsoSilent, request.correlationId);
