...
function Test-DuckDb {
    ...
    try {
        ...
        Write-PSFMessage $_.Exception.Message -Level Error

        # Check for specific DuckDB initialization error that indicates missing Visual C++ Redistributable
        if ($_.Exception.Message -like "*The type initializer for 'DuckDB.NET.Data.DuckDBConnectionStringBuilder' threw an exception*") {
            Write-Host
