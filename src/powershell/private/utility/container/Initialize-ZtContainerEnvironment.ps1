function Initialize-ZtContainerEnvironment {
    ...
    if ($issues.Count -eq 0) {
        ...
        foreach ($issue in $issues) {
            Write-PSFMessage -Message $issue -Level Warning
        }
        if (-not $browserAuthReady) {
            Write-Host -Object '    ⚠️ Browser authentication may not work. Device code flow will be offered.' -ForegroundColor Yellow
        }
    }
    Write-Host
    #endregion
