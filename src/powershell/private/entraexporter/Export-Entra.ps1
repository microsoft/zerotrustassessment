...
Function Export-Entra {
    ...
    foreach ($item in $ExportSchema) {
        ...
        if ( ($typeMatch)) {
            ...
            $spacer = ''
            if ($hasParents) {
                $spacer = ''.PadRight($Parents.Count + 3, ' ') + $Parents[$Parents.Count - 1]
            }
            ...
            if ($command) {
                ...
                try {
                    $resultItems = Invoke-ZtGraphRequest $graphUri -Filter (Get-ObjectProperty $item 'Filter') -Select (Get-ObjectProperty $item 'Select') -QueryParameters (Get-ObjectProperty $item 'QueryParameters') -ApiVersion $apiVersion  -OutputFilePath $outputFileName
                }
                catch {
                    $e = ""
            ...
            if ($outputFileName -notmatch "\.json$") {
                foreach ($resultItem in $resultItems) {
                    ...
                    $itemOutputFileName = Join-Path $outputFileName 'Children' $resultItem.id
