...
function Test-Assessment-21964 {
    ...
    if (-not $query1) {
        ...
        foreach ($action in $unprotectedActions) {
            if ($action.authenticationContextId) {
                $authenticationContextId = $($action.authenticationContextId)
            }
            else {
    ...
    $testResultMarkdown = $null
    $testResultMarkdown = "Found Conditional Access policies below configurations `n`n"
    $testResultMarkdown += "| Display Name | State | Authentication Strength | Device Filters | SignIn Frequency |`n"
    $testResultMarkdown += "| :--- | :--- | :--- | :--- | :--- |`n"

    foreach ($policy in $($policiesPerAction | Sort-Object -Unique -Property id)) {
    ...
    if ($passed) {
        $testResultMarkdown = $null
        $testResultMarkdown = 'Protected actions have been enabled for Conditional Access policy create, update and delete permissions, and Authentication Context update permission. Common stronger Conditional Access policies are enabled for each permission per protected actions documentation.'
    }
    else {
        $testResultMarkdown = 'protected actions have been applied to Conditional Access policy create, update and delete permissions, and Authentication Context update permission. User to confirm whether Conditional Access controls meet organizational security requirements.'
