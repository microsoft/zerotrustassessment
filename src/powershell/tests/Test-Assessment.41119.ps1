...
function Test-Assessment-41119 {
    ...
    foreach ($policy in $settingsCatalogPolicies) {
        ...
        # surface as Investigate rather than a false Pass/Fail (isAssigned is not returned by default).
        $assignmentsProp = $policy.PSObject.Properties['assignments']
        # An absent property or a null value is unusable evidence (Unknown); a real (possibly empty)
        # collection is usable — an empty array is a definite Unassigned.
        $assignmentsUsable = ($null -ne $assignmentsProp) -and ($null -ne $assignmentsProp.Value)
