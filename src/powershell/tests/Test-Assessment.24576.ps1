...
function Test-Assessment-24576 {
    ...
    # Query: Retrieve all Windows Health Monitoring device configuration policies
    $sql = @"
    SELECT id, displayName, assignments
    FROM WindowsHealthMonitoringConfiguration
    WHERE "@odata.type" = '#microsoft.graph.windowsHealthMonitoringConfiguration'
"@

    $windowsHealthMonitoringPolicies = Invoke-DatabaseQuery -Database $Database -Sql $sql -AsCustomObject
