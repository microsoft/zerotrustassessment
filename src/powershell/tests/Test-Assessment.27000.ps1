...
function Test-Assessment-27000 {
    ...
    $filteringPolicies = Invoke-ZtGraphRequest -RelativeUri 'networkAccess/filteringPolicies' -QueryParameters @{ '$expand' = 'policyRules' } -ApiVersion beta
    ...
    $filteringProfiles = Invoke-ZtGraphRequest -RelativeUri 'networkAccess/filteringProfiles' -QueryParameters @{ '$expand' = 'policies($expand=policy)' } -ApiVersion beta
        ...
        foreach ($policy in $policiesCoveringCategory) {
            ...
            foreach ($linkedProfile in $linkedProfiles) {
                ...
                foreach ($policyLink in $filteringProfile.policies) {
                    ...
                    # The policy link doesn't have an action property, but policy.action does
