...
|**Sensitive conversations of known nature**|Custom Trainable classifier trained based on confirmed samples| |Idenfity in Content Explorer documents with labels that are relevant to those subjects, extract them and use them to train a custom classifier. |
