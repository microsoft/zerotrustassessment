...
namespace ZeroTrustAssessment.DocumentGenerator
{
    ...
    public class ZtWorkbook
    {
        ...
        public async Task<Roadmap> GetRoadmapAsync()
        {
            ...
            var sheetIdentity = GetWorksheet(_workbook, ZtSheets.WorkshopIdentity);
            roadmap.TenantName = sheetIdentity.TextBoxes["RoadmapIdentityTenantName"].Text.Replace(Environment.NewLine, string.Empty);
            roadmap.TenantId = sheetIdentity.TextBoxes["RoadmapIdentityTenantId"].Text.Replace(Environment.NewLine, string.Empty);
