...
Param (
	[string[]]
	$CommandPath = @(
		"$($global:__testData.ModuleRoot)\public"
		# Uncomment to enforce help for all internal commands
		# "$($global:__testData.ModuleRoot)\private"
	),

	[string]
	$ModuleName = $global:__testData.ModuleName,

	[string]
	$ExceptionsFile = "$($global:__testData.TestRoot)\general\Help.Exceptions.ps1"
)
...
foreach ($command in $commands) {
    ...
    if ($global:__testData.Tests.Help.Exempt -contains $commandName) { continue }
	...
	Describe "Test help for $commandName" {
        ...
        Context "Test parameter help for $commandName" {
            ...
            foreach ($parameter in $parameters) {
                ...
                if ($global:__testData.Tests.Help.SkipParameterType[$commandName] -contains $parameterName) { continue }
                ...
                elseif ($parameter.ParameterType.FullName -in $global:__testData.Tests.Help.EnumeratedArrays) {
                    # Enumerations often have issues with the typename not being reliably available
